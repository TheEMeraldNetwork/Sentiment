...
def generate_html():
    ...
    # Read the master output and analyst consensus
    df = pd.read_csv(project_root / 'results/d_master_output.csv')
    
    try:
        analyst_df = pd.read_csv(project_root / 'results/c_analyst_consensus.csv')
        # Merge with analyst data
        df = df.merge(
            analyst_df[['ticker', 'target_median', 'num_analysts', 'potential_return', 'recommendation']],
            on='ticker',
            how='left'
        )
    except Exception as e:
        print(f"Warning: Could not load analyst consensus data: {e}")
    ...
    for _, row in df.iterrows():
        ...
        # Format target price and return with proper handling of missing values
        target_price = f"${row['target_median']:.2f}" if pd.notna(row.get('target_median')) else ''
        potential_return = f"{row['potential_return']:.1f}%" if pd.notna(row.get('potential_return')) else ''
        analysts = str(int(row['num_analysts'])) if pd.notna(row.get('num_analysts')) else ''
